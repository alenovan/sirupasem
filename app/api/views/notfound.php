<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Gull - Laravel + Bootstrap 4 admin template</title>
    <link href="https://fonts.googleapis.com/css?family=Nunito:300,400,400i,600,700,800,900" rel="stylesheet">
    <link rel="stylesheet" href="<?php echo base_url(); ?>assets/admin/styles/css/themes/lite-purple.min.css">
    <link rel="stylesheet" href="<?php echo base_url(); ?>assets/admin/styles/vendor/perfect-scrollbar.css">
</head>

<body>
<div class="not-found-wrap text-center">
    <h1 class="text-60">
        404
    </h1>
    <p class="text-36 subheading mb-3">Mohon Maaf!</p>
    <p class="mb-5  text-muted text-18">Anda Tidak Punya Akses pada halaman ini</p>
    <a class="btn btn-lg btn-primary btn-rounded" href="<?php echo base_url().index_page(); ?>dashboard">Go back to Dashboard</a>
</div>
</body>

</html>